...
SystemStatus::SystemStatus(uint8_t apin_batt)
  : pin_batt(apin_batt) {
}
...
int SystemStatus::getChipTemperatureCelsius(int bandgap_voltage, float offset = 0, int vcc = 0) {
  int i;
  int t_celsius;
  uint8_t vccIndex;
  float rawTemp;
  ...
  if (!vcc) {
    vcc = getVCC(bandgap_voltage);
  }


  //index 0..13 for vcc 1.7 ... 3.0
  vccIndex = min(max(17, (uint8_t)(vcc * 10)), 30) - 17;

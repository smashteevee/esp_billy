...
void setup() {
  ...
  Serial.print("EEPROM[15] = ");
  Serial.println(EEPROM[15]);
  Serial.print("EEPROM[16] = ");
  Serial.println(EEPROM[16]);
  Serial.print("EEPROM[17] = ");
  Serial.println(EEPROM[17]);
  Serial.print("EEPROM[18] = ");
  Serial.println(EEPROM[18]);
...
void loop() {
  ...
  if (vcc > CUTOFF_VOLTAGE) {
    ...
    if (readVoltage) {
      ...
      // TODO: WAIT FOR READY THEN SEND instead of HARDCODED delays
      delay(4000);  // ESP takes 1-3s from boot to connect
      wdt_reset(); // Pat the dog
      ...
      // TODO: REPLACE DELAY WITH READY SIGNAL FROM ESP BILLY
      wdt_reset();  // Pat the dog
